...
export const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    if (!chatId) return next(new ErrorHandler("Please provide chatId", 404));
    ...
    return res.status(201).json({
        success: true,
        message: "sendAttachments called",
        messageForRealTime,
    });
});
...
export const getChatDetails = TryCatch(async (req, res, next) => {
    if (req.query?.populate === "true") {
        // console.log("Getting Chats with populating");
...
export const renameChat = TryCatch(async (req, res, next) => {
    const chatId = req.params.id;
    ...
    if (chat.creator.toString() !== userId.toString())
        return next(
            new ErrorHandler("You are not allowed to rename the chat", 403)
        );
...
export const getMessages = TryCatch(async (req, res, next) => {
    ...
    if (!chat.members.includes(userId.toString()))
        return next(
            new ErrorHandler("You are not allowed to see the chat", 403)
        );
